...
export const getLineGuideStops = (
  ...
  // Vertical: Canvas edges (start and end of timeline conceptually, though infinite)
  // For now, let's just add 0.
  vertical.push(getStopsForObject(0, 0, 0, canvas.height || 0));
  ...
  canvas
    ...
    .filter(
      (o) =>
        o.visible &&
        (o as any).clipId &&
        (o as any).clipId !== 'drag-guide' &&
        (o as any).type !== 'placeholder'
    ) // Changed elementId to clipId and excluded placeholder
    .forEach((guideObject) => {
      if (
