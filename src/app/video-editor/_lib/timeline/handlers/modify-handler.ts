...
function updateClipTimeLocally(
  ...
  // Logic to update clips map locally if needed for optimistic UI
  // For now, we rely on the event emission to update the store/engine
  console.log(
    `[ModifyHandler] Updating local time for ${clipId} to ${newDisplayFrom}us`
  );
}
...
export function handleTrackRelocation(timeline: TimelineEngine, options: any) {
  ...
  console.log('[ModifyHandler] handleTrackRelocation triggered', target);
  ...
  if (droppedTarget && (droppedTarget as any).isHelper) {
    ...
    console.log('[ModifyHandler] Dropped on Helper', helper.separatorIndex);
    ...
    if (trackRegion) {
      console.log(
        `[ModifyHandler] Drop on Track ${trackRegion.id} detected via region`
      );
      ...
      if (hasOverlap) {
        console.log(`[ModifyHandler] Overlap detected -> New Track`);
        ...
        console.log(`[ModifyHandler] No overlap -> Move to Track`);
        // Force visual snap
        if (target) {
          target.set({
            top: trackRegion.top,
    ...
    console.log('[ModifyHandler] Dropped in empty space');
...
export function handleClipModification(timeline: TimelineEngine, options: any) {
  ...
  console.log('[ModifyHandler] handleClipModification triggered');
  ...
  if (targetAny.type === 'activeSelection' && targetAny._objects) {
    ...
    if (clips.length > 0) {
      console.log(
        `[ModifyHandler] Emitting clips:modified for ${clips.length} clips`
      );
    ...
    console.log(`[ModifyHandler] Emitting clip:modified for ${clipId}`, {
      displayFrom,
      duration,
    });
