export type Language = 'es' | 'en';

export const translations = {
  es: {
    'media.upload': 'Cargar',
    'media.create_ai': 'Crear assets AI',
    'media.generate_unique': 'Genera contenido único',
    'image.upload': 'Cargar imagen',
    'image.generate_ai': 'Generar AI Image',
    'image.create_unique': 'Crea imágenes únicas',
    title: 'Ingeniero de Sistemas e Informática',
    bio: 'Profesional apasionado por la tecnología con más de 6 años de experiencia en ciberseguridad, desarrollo e implementación. Especializado en crear soluciones óptimas y liderar equipos de alto rendimiento.',
    phone: 'Teléfono',
    email: 'Correo',
    location: 'Ubicación',
    githubInsights: 'GitHub Insights',
    repos: 'Proyectos',
    stars: 'Estrellas',
    followers: 'Seguidores',
    topRepo: 'Top Repo Stars',
    annualActivity: 'Actividad Anual',
    online: 'En línea',
    skills: 'Habilidades',
    personalProjects: 'Proyectos Personales',
    professionalExperience: 'Experiencia Profesional',
    certifications: 'Certificaciones',
    references: 'Referencias',
    launchDemo: 'Lanzar Demo',
    viewCode: 'Ver Código',
    verProyecto: 'Ver proyecto online',
    recentProjects: 'Proyectos Recientes',
    videoEditorV1Title: 'React Video Editor - Remotion V1',
    videoEditorV2Title: 'Next Video Editor - openvideo V2',
    videoEditorV1Desc:
      'Un editor de video robusto construido con React y Remotion. Permite la edición en tiempo real de líneas de tiempo, capas y efectos con una interfaz profesional e intuitiva.',
    videoEditorV2Desc:
      'Edición de video de nueva generación impulsada por openvideo y Fabric.js. Incluye gestión de estado avanzada, renderizado optimizado y un lienzo interactivo de alto rendimiento.',
    scrollHint: 'Desliza para ver actividad anterior',
    university: 'Univ. Tecnológica del Perú',
    degree: 'Ingeniería de Sistemas e Informática',
    footer:
      '© 2026 Pablito Silva Inca • Construido con Next.js, Shadcn & Antigravity',
    expCliquify:
      'Liderazgo en el desarrollo de editores de imágenes y video avanzados. Implementación de tecnologías complejas de manipulación de lioenzos.',
    expMyDesign:
      'Desarrollo de editores de imágenes en Vue JS con renderizado optimizado a través de AWS Lambda. Integración de herramientas de diseño interactivas.',
    expDrawify:
      'Desarrollo de herramientas de diseño con integraciones de AWS S3 y pasarelas de pago Stripe. Control de versiones y despliegue continuo.',
    expTelefonica:
      'Operador avanzado de ciberseguridad en el área EDR. Despliegue, soporte y monitoreo del producto Cortex XDR para clientes corporativos.',
    roleFrontend: 'Desarrollador Front-end',
    rolePM: 'Project Manager',
    roleFullstack: 'Desarrollador Full-stack Sr',
    roleTelecom: 'Analista de Telecomunicaciones',
    historyHint: 'Usa tu dedo para navegar',
    recentActivity: 'Actividad reciente',
    education: 'Educación',
    roleAnalyst: 'Analista de Ciberseguridad',
    'updates.title': 'Novedades',
    'updates.improved_features': 'Funciones Mejoradas',
    'updates.ai_features': 'Inteligencia Artificial',
    'updates.latest_updates':
      'Explora las últimas mejoras y próximas funciones',
    'updates.sync_logic': 'Sincronización de línea de tiempo',
    'updates.sync_logic_desc':
      'Sincronización bidireccional avanzada entre el lienzo de la línea de tiempo y el estado del Studio, garantizando actualizaciones en tiempo real.',
    'updates.track_management': 'Multi-pista y Alineación',
    'updates.track_management_desc':
      'Soporte para la creación de múltiples pistas a partir de selecciones y alineación inteligente de clips a estas pistas.',
    'updates.zoom_seeking': 'Zoom y Búsqueda',
    'updates.zoom_seeking_desc':
      'Zoom fluido en la línea de tiempo y búsqueda precisa a través de la regla interactiva.',
    'updates.clip_tools': 'Herramientas de edición de clips',
    'updates.clip_tools_desc':
      'Nuevas herramientas para dividir, duplicar y eliminar clips con soporte para atajos de teclado.',
    'updates.smart_sync': 'Sincronización Inteligente',
    'updates.smart_sync_desc':
      'Corrección de coordenadas absolutas en multiselecciones y sincronización precisa con el Studio.',
    'updates.keyboard_zoom': 'Control de Zoom por Teclado',
    'updates.keyboard_zoom_desc':
      'Usa Ctrl/Meta + Rueda del ratón para un zoom fluido y rápido en la línea de tiempo.',
    'updates.performance_ui': 'Rendimiento UI Optimizado',
    'updates.performance_ui_desc':
      'Reducción de renderizados innecesarios y mayor fluidez durante la reproducción.',
    'updates.close': '¡Entendido!',
    'updates.core_features': 'Funciones Core',
    'updates.video_support': 'Soporte de Video',
    'updates.video_support_desc':
      'Carga y edición de archivos de video con vista previa en tiempo real y recorte preciso.',
    'updates.image_support': 'Soporte de Imágenes',
    'updates.image_support_desc':
      'Integración de imágenes estáticas con duración ajustable y manipulación directa en el lienzo.',
    'updates.state_control': 'Control de Estado',
    'updates.state_control_desc':
      'Gestión robusta de la línea de tiempo con guardado automático y coherencia total entre capas.',
    'updates.coming_soon': 'Próximamente',
    'updates.ai_voiceover': 'Locución por IA (TTS)',
    'updates.voice_ai_pro': 'Voz AI Pro (Buscador + 22 Voces)',
    'updates.voice_ai_pro_desc':
      'Buscador inteligente por etiquetas (cartoon, slow, etc.), catálogo extendido a 22 voces y pre-escucha bilingüe (EN/ES) antes de añadir al timeline.',
    'updates.transitions': 'Efectos de Transición',
    'updates.audio_mixing': 'Mezcla de Audio Avanzada',
    'updates.filters_effects': 'Filtros y Efectos Visuales',
    'updates.export_formats': 'Nuevos Formatos de Exportación',
    'updates.stt': 'Voz a Texto (Speech to Text)',
    'updates.captions': 'Generación de Subtítulos',
    'updates.script_to_video': 'Guion a Video por IA',
    'updates.link_to_video': 'Enlace a Video por IA',
    'updates.i18n': 'Soporte Bilingüe (EN/ES)',
    'updates.i18n_desc':
      'Ahora toda la interfaz está disponible en Español e Inglés con cambio instantáneo.',
    'updates.ai_stability': 'Estabilidad Asistente AI',
    'updates.ai_stability_desc':
      'Migración estratégica a us-east4 y manejo de cuotas mejorado. Mayor estabilidad en el procesamiento de videos pesados y streaming de Gemini.',
    'updates.corner_radius_fix': 'Fix de Radio de Borde',
    'updates.corner_radius_fix_desc':
      'Se ha corregido un problema que impedía aplicar el radio de borde correctamente con valor 0, forzando un mínimo de 1.',
    'updates.ai_assets_desc':
      'Generación impulsada por LangChain con Vision Proxy. Transformación de estilo profunda y consistencia visual mejorada.',
    'updates.ai_highlights': 'AI Highlights v3.5 (LangChain)',
    'updates.ai_highlights_desc':
      'Análisis avanzado con LangChain Vertex AI. Mayor precisión en la detección de momentos y control de duración de clips.',
    'updates.ai_captions': 'Subtítulos por IA v1',
    'updates.ai_captions_desc':
      'Generación y quemado de subtítulos automáticamente usando Deepgram y Gemini. Incluye una nueva vista de edición y previsualizaciones de clips.',
    'updates.transitions_stability': 'Estabilidad en Transiciones',
    'updates.transitions_stability_desc':
      'Se ha resuelto el comportamiento inconsistente al añadir transiciones en uniones complejas de pistas.',
    'updates.export_performance': 'Rendimiento de Exportación',
    'updates.export_performance_desc':
      'Algunos usuarios pueden experimentar lentitud en exportaciones largas. Estamos optimizando nuestros renderizadores en la nube.',
    'updates.auth': 'Soporte de Autenticación',
    'updates.auth_desc':
      'Inicio de sesión seguro mediante Google, GitHub o Email impulsado por Supabase.',
    'updates.access_control': 'Control de Acceso IA',
    'updates.access_control_desc':
      'Las funciones de IA (Highlights, Assets y Subtítulos) ahora están protegidas por permisos de usuario y validación de API Keys en tiempo real con modo demo para visitantes.',
    'updates.highlights_tuning': 'Ajuste de Precisión en Clips',
    'updates.highlights_tuning_desc':
      'Pre-procesamiento local con FFmpeg (frames + audio), reduciendo drásticamente el uso de tokens y acelerando el análisis de Gemini.',
    'updates.langchain_orchestration': 'Orquestación LangChain',
    'updates.langchain_orchestration_desc':
      'Nueva infraestructura basada en LangChain para una lógica de IA más robusta, modular y escalable en todo el Studio.',
    'updates.vision_proxy': 'Vision Proxy (Antigravity)',
    'updates.vision_proxy_desc':
      'Análisis inteligente de imágenes de referencia que permite romper el sesgo de píxeles y lograr transformaciones de estilo artísticas reales.',
    'captions.back': 'Volver',
    'captions.setup_title': 'Auto Subtítulos',
    'captions.list_title': 'Editar Subtítulos',
    'captions.regenerate': 'Regenerar Subtítulos',
    'captions.no_audio_error':
      'El video seleccionado no tiene pista de audio. No se pueden generar subtítulos.',
    'captions.processing': 'Procesando Audio...',
    'captions.system_message': 'Mensaje del Sistema',
    'captions.transcription_error': 'Error de Transcripción',
    'captions.alignment_complete': 'Alineación automática completada',
    'highlights.title': 'AI Highlights',
    'highlights.description':
      'Extrae automáticamente los mejores momentos de tus videos largos.',
    'highlights.upload_title': 'Sube un video largo',
    'highlights.upload_subtitle':
      'IA identificará los mejores momentos para shorts',
    'highlights.extract_btn': 'Identificar Momentos con IA',
    'highlights.processing': 'Analizando video con Gemini...',
    'highlights.suggested': 'Momentos Sugeridos',
    'highlights.add_all': 'Agregar todos a la línea de tiempo',
    'highlights.target_duration': 'Duración objetivo por clip',
    'highlights.range_short': 'Corto (5 a 30 segundos)',
    'highlights.range_medium': 'Medio (30 a 60 segundos)',
    'highlights.range_long': 'Largo (1 a 3 minutos)',
    'highlights.url_label': 'Pegar link de YouTube',
    'highlights.url_placeholder': 'https://www.youtube.com/watch?v=...',
    'highlights.url_btn': 'Descargar y Analizar',
    'highlights.add_to_timeline': 'Agregar a Timeline',
    'highlights.status_init': 'Iniciando procesamiento...',
    'highlights.status_uploading': 'Subiendo parte a Gemini...',
    'highlights.status_analyzing': 'Gemini analizando parte...',
    'highlights.status_optimizing': 'Optimizando parte...',
    'highlights.status_split': 'Video dividido en partes.',
    'highlights.status_complete': '¡Análisis completado!',
    'highlights.change_video': 'Cambiar',
    'highlights.minutes': 'minutos',
    'highlights.seconds': 'segundos',
    'highlights.status_finalizing': 'Finalizando detalles...',
    'highlights.status_wait': 'Esto puede tomar unos momentos...',
    'highlights.clips_count': 'clips',
    'highlights.status_verifying': 'Verificando permisos...',
    'highlights.status_loading_tools': 'Cargando herramientas...',
    'highlights.status_trimming': 'Recortando minutos',
    'highlights.status_analyzing_range': 'Analizando minutos',
    'highlights.status_quota_wait': 'Saturación regional. Reintentando en',
    'highlights.status_breathing': 'Respirando para liberar cuota...',
    'highlights.status_error': 'Error en fragmento',
    'highlights.demo_mode': 'Modo Demo',
    'highlights.demo_title': 'Contenido Protegido',
    'highlights.demo_description':
      'Esta función requiere una clave de API activa. Contacta al desarrollador para una demostración personalizada.',
    'highlights.current_features': 'Funciones Actuales',
    'highlights.overview': 'Resumen',
    'updates.issues': 'Problemas Conocidos',
    'updates.transitions_fix': 'Incidencia en Transiciones',
    'updates.transitions_fix_desc':
      'Se ha detectado un comportamiento inconsistente al añadir transiciones en uniones complejas. Nuestro equipo está trabajando en una solución prioritaria.',
    'header.export': 'Exportar',
    'header.logout': 'Cerrar Sesión',
    'header.import_json': 'Importar desde JSON',
    'header.export_json': 'Exportar a JSON',
    'header.json_menu': 'JSON',
    'header.undo': 'Deshacer',
    'header.redo': 'Rehacer',
    'transitions.title': 'Transiciones',
    'transitions.editing_union': 'Editando Unión',
    'effects.title': 'Efectos',
    'text.basic': 'Básico',
    'text.add_text': 'Agregar texto',
    'text.add_captions': 'Agregar captions',
    'toolbox.project_name': 'Proyecto de Pablito',
    'toolbox.coming_soon': 'Próximamente...',
    'modal.ai_assets': 'Assets AI',
    'modal.ai_images': 'Generación de Imágenes AI',
    'menu.media': 'Media',
    'menu.audio': 'Audio',
    'menu.text': 'Texto',
    'menu.image': 'Imagen',
    'menu.effects': 'Efectos',
    'menu.transitions': 'Transiciones',
    'menu.highlights': 'AI Clips',
    'menu.captions': 'Subtítulos',
    'menu.ai_assets': 'AI Assets',
    'menu.voice_ai': 'Voz AI',
    'voice_ai.description': 'Genera locuciones profesionales con ElevenLabs.',
    'voice_ai.input_method': 'Método de Entrada',
    'voice_ai.tts': 'Texto a Voz',
    'voice_ai.sts': 'Audio a Voz',
    'voice_ai.content_script': 'Contenido del Guion',
    'voice_ai.content_audio': 'Audio de Referencia',
    'voice_ai.placeholder_text':
      'Escribe aquí el texto que la IA debe locutar...',
    'voice_ai.characters': 'caracteres',
    'voice_ai.volume': 'Volumen Maestro',
    'voice_ai.upload_audio': 'Subir archivo de audio',
    'voice_ai.select_voice': 'Seleccionar Voz',
    'voice_ai.settings': 'Ajustes',
    'voice_ai.generate': 'Generar Locución',
    'voice_ai.search_placeholder':
      'Buscar por nombre o etiquetas (cartoon, animate...)',
    'voice_ai.generated_results': 'Resultados Generados',
    'voice_ai.add_to_timeline': 'Añadir a Timeline',
    'voice_ai.play': 'Reproducir',
    'voice_ai.stop': 'Detener',
    'voice_ai.voice_desc_soft': 'Narración suave',
    'voice_ai.voice_desc_sarah': 'Madura y confiada',
    'voice_ai.voice_desc_charlie': 'Enérgica y alegre',
    'voice_ai.voice_desc_george': 'Narrador cautivador',
    'voice_ai.voice_desc_alice': 'Educadora profesional',
    'voice_ai.voice_desc_matilda': 'Profesional y seria',
    'voice_ai.voice_desc_will': 'Optimista y relajado',
    'voice_ai.voice_desc_eric': 'Cercana y confiable',
    'voice_ai.voice_desc_brian': 'Profunda y reconfortante',
    'voice_ai.voice_desc_bill': 'Voz sabia y madura',
    'voice_ai.voice_desc_roger': 'Elegante y casual',
    'voice_ai.voice_desc_laura': 'Excéntrica y entusiasta',
    'voice_ai.voice_desc_callum': 'Bribón ronco',
    'voice_ai.voice_desc_river': 'Relajada y neutral',
    'voice_ai.voice_desc_harry': 'Guerrero feroz',
    'voice_ai.voice_desc_liam': 'Creador enérgico',
    'voice_ai.voice_desc_jessica': 'Juguetona y brillante',
    'voice_ai.voice_desc_bella': 'Profesional y brillante',
    'voice_ai.voice_desc_chris': 'Encantador y natural',
    'voice_ai.voice_desc_daniel': 'Locutor estable',
    'voice_ai.voice_desc_lily': 'Actriz aterciopelada',
    'voice_ai.voice_desc_adam': 'Dominante y firme',
    'ai_assets.title': 'Generador AI',
    'ai_assets.description': 'Genera imágenes y videos únicos usando IA.',
    'ai_assets.upload_image': 'Subir Imagen',
    'ai_assets.upload_video': 'Subir Video',
    'ai_assets.generate_img': 'Generar Imagen',
    'ai_assets.generate_vid': 'Generar Video',
    'ai_assets.generate_btn': 'Generar',
    'ai_assets.type': 'Tipo de Contenido',
    'ai_assets.type_image': 'Imagen',
    'ai_assets.type_video': 'Video',
    'ai_assets.select_source': 'Selecciona un archivo para empezar',
    'ai_assets.generating': 'Generando contenido...',
    'ai_assets.add_to_project': 'Añadir al Proyecto',
    'ai_assets.video_duration': 'Duración del Video',
    'ai_assets.duration_1m': '1 minuto',
    'ai_assets.duration_3m': '3 minutos',
    'ai_assets.duration_5m': '5 minutos',
    'ai_assets.style': 'Estilo Visual',
    'ai_assets.style_realistic': 'Realista',
    'ai_assets.style_scientific_glow': 'Anatomía Brillante',
    'ai_assets.style_manga': 'Manga',
    'ai_assets.style_cinematic': 'Cinemático',
    'ai_assets.style_3d': '3D Render',
    'ai_assets.style_cyberpunk': 'Cyberpunk',
    'ai_assets.style_cartoon': 'Cartoon',
    'ai_assets.style_pixel': 'Pixel Art',
    'ai_assets.style_watercolor': 'Acuarela',
    'ai_assets.style_oil': 'Óleo',
    'ai_assets.style_sketch': 'Bosquejo',
    'ai_assets.style_vintage': 'Vintage',
    'ai_assets.aspect_ratio': 'Relación de Aspecto',
    'ai_assets.quality': 'Calidad',
    'ai_assets.quality_standard': 'Estándar',
    'ai_assets.quality_high': 'Alta',
    'ai_assets.quality_hd': 'HD',
    'timeline.split': 'Dividir (S)',
    'timeline.duplicate': 'Duplicar (D)',
    'timeline.delete': 'Eliminar (Del)',
    'timeline.snapping': 'Ajuste (N)',
    'chat.assistant': 'Asistente AI',
    'chat.mode_demo': 'Modo Demo',
    'chat.gemini': 'Potenciado por Gemini',
    'chat.how_assist': '¿En qué puedo ayudarte?',
    'chat.placeholder':
      'Puedo ayudarte con ediciones, efectos o explicar funciones del editor.',
    'chat.thinking': 'Pensando...',
    'chat.type_message': 'Escribe un mensaje...',
    'chat.inaccurate':
      'El asistente AI puede proporcionar información inexacta',
    'chat.adding_text': 'Agregando texto',
    'chat.updating_style': 'Actualizando estilo del texto...',
    'chat.updating_media_style': 'Actualizando estilo del medio...',
    'text.apply_hardcoded': 'Aplicar Estilo Hardcoded',
    'image.apply_hardcoded': 'Aplicar Estilo Hardcoded (Imagen)',
    'login.subtitle': 'Toda la potencia de la IA para tus videos.',
    'login.title': 'Editor de Video AI',
    'login.description': 'Inicia sesión con tu cuenta de Google para comenzar',
    'login.google': 'Continuar con Google',
    'login.github': 'GitHub',
    'login.or_continue': 'O CONTINÚA CON EMAIL',
    'login.email': 'Correo electrónico',
    'login.password': 'Contraseña',
    'login.signin': 'Iniciar Sesión',
    'login.signup': 'Registrarse',
    'login.privacy':
      'Al iniciar sesión, aceptas nuestra política de privacidad.',
    'login.ai_notice':
      'El inicio de sesión es necesario para llevar un control de los servicios de IA, ya que no están habilitados de forma pública para todos.',
    'login.rights': 'Todos los derechos reservados.',
    'login.try_without_login': 'Probar editor sin iniciar sesión',
    'login.try_without_login_note':
      'Las funciones de IA no estarán disponibles',
    'header.guest_mode': 'Modo Invitado',
    'header.sign_in': 'Iniciar Sesión',
    'header.guest_banner': 'Inicia sesión para desbloquear las funciones de IA',
    'header.shortcuts': 'Atajos de teclado',
    'shortcuts.title': 'Atajos de Teclado',
    'shortcuts.global': 'Global',
    'shortcuts.timeline': 'Línea de Tiempo',
    'shortcuts.canvas': 'Canvas',
    'shortcuts.undo': 'Deshacer',
    'shortcuts.redo': 'Rehacer',
    'shortcuts.play_pause': 'Reproducir / Pausar',
    'shortcuts.delete': 'Eliminar',
    'shortcuts.duplicate': 'Duplicar',
    'shortcuts.copy': 'Copiar',
    'shortcuts.paste': 'Pegar',
    'shortcuts.select_all': 'Seleccionar todo',
    'shortcuts.save_json': 'Guardar proyecto (JSON)',
    'shortcuts.export_video': 'Exportar video',
    'shortcuts.deselect': 'Deseleccionar',
    'shortcuts.split': 'Dividir en playhead',
    'shortcuts.zoom_in': 'Acercar timeline',
    'shortcuts.zoom_out': 'Alejar timeline',
    'shortcuts.go_to_start': 'Ir al inicio',
    'shortcuts.go_to_end': 'Ir al final',
    'shortcuts.prev_frame': 'Cuadro anterior',
    'shortcuts.next_frame': 'Cuadro siguiente',
    'shortcuts.move_up': 'Mover arriba 1px',
    'shortcuts.move_down': 'Mover abajo 1px',
    'shortcuts.move_left': 'Mover izquierda 1px',
    'shortcuts.move_right': 'Mover derecha 1px',
    'shortcuts.move_5px': 'Mover 5px',
  },
  en: {
    'media.upload': 'Upload',
    'media.create_ai': 'Create AI Assets',
    'media.generate_unique': 'Generate unique content',
    'image.upload': 'Upload Image',
    'image.generate_ai': 'Generate AI Image',
    'image.create_unique': 'Create unique images',
    title: 'Systems and Informatics Engineer',
    bio: 'Technology enthusiast professional with over 6 years of experience in cybersecurity, development, and implementation. Specialized in creating optimal solutions and leading high-performance teams.',
    phone: 'Phone',
    email: 'Email',
    location: 'Location',
    githubInsights: 'GitHub Insights',
    repos: 'Projects',
    stars: 'Stars',
    followers: 'Followers',
    topRepo: 'Top Repo Stars',
    annualActivity: 'Annual Activity',
    online: 'Online',
    skills: 'Skills',
    personalProjects: 'Personal Projects',
    professionalExperience: 'Professional Experience',
    certifications: 'Certifications',
    references: 'References',
    launchDemo: 'Launch Demo',
    viewCode: 'View Code',
    verProyecto: 'View project online',
    recentProjects: 'Recent Projects',
    videoEditorV1Title: 'React Video Editor - Remotion V1',
    videoEditorV2Title: 'Next Video Editor - openvideo V2',
    videoEditorV1Desc:
      'A robust video editor built with React and Remotion. Enables real-time editing of timelines, layers, and effects with a professional and intuitive interface.',
    videoEditorV2Desc:
      'Next-generation video editing powered by openvideo and Fabric.js. Features advanced state management, optimized rendering, and a high-performance interactive canvas.',
    scrollHint: 'Swipe to see previous activity',
    university: 'Technological University of Peru',
    degree: 'Systems and Informatics Engineering',
    footer:
      '© 2026 Pablito Silva Inca • Built with Next.js, Shadcn & Antigravity',
    expCliquify:
      'Leading the development of advanced image and video editors. Implementation of complex canvas manipulation technologies.',
    expMyDesign:
      'Development of image editors in Vue JS with optimized rendering via AWS Lambda. Integration of interactive design tools.',
    expDrawify:
      'Development of design tools with AWS S3 integrations and Stripe payment gateways. Version control and continuous deployment.',
    expTelefonica:
      'Advanced cybersecurity operator in the EDR area. Deployment, support, and monitoring of the Cortex XDR product for corporate clients.',
    roleFrontend: 'Front-end Developer',
    rolePM: 'Project Manager',
    roleFullstack: 'Sr. Full-stack Developer',
    roleTelecom: 'Telecommunications Analyst',
    historyHint: 'Use your finger to navigate',
    recentActivity: 'Recent activity',
    education: 'Education',
    roleAnalyst: 'Cybersecurity Analyst',
    'updates.title': "What's New",
    'updates.improved_features': 'Improved Features',
    'updates.ai_features': 'Artificial Intelligence',
    'updates.latest_updates':
      'Explore the latest improvements and upcoming features',
    'updates.sync_logic': 'Timeline Synchronization',
    'updates.sync_logic_desc':
      'Advanced bidirectional synchronization between the timeline canvas and the Studio state, ensuring real-time updates.',
    'updates.track_management': 'Multi-track & Alignment',
    'updates.track_management_desc':
      'Support for creating multiple tracks from selections and smart alignment of clips to these tracks.',
    'updates.zoom_seeking': 'Zoom & Seeking',
    'updates.zoom_seeking_desc':
      'Smooth zooming in the timeline and precise seeking through the interactive ruler.',
    'updates.clip_tools': 'Clip Editing Tools',
    'updates.clip_tools_desc':
      'New tools for splitting, duplicating, and deleting clips with keyboard shortcut support.',
    'updates.smart_sync': 'Smart Synchronization',
    'updates.smart_sync_desc':
      'Absolute coordinate fixes for multi-selections and precise Studio synchronization.',
    'updates.keyboard_zoom': 'Keyboard Zoom Control',
    'updates.keyboard_zoom_desc':
      'Use Ctrl/Meta + Mouse Wheel for smooth and fast timeline zooming.',
    'updates.performance_ui': 'Optimized UI Performance',
    'updates.performance_ui_desc':
      'Reduced unnecessary re-renders and improved smoothness during playback.',
    'updates.close': 'Got it!',
    'updates.core_features': 'Core Features',
    'updates.video_support': 'Video Support',
    'updates.video_support_desc':
      'Upload and edit video files with real-time preview and precise trimming.',
    'updates.image_support': 'Image Support',
    'updates.image_support_desc':
      'Integration of static images with adjustable duration and direct canvas manipulation.',
    'updates.state_control': 'State Control',
    'updates.state_control_desc':
      'Robust timeline management with automatic saving and total coherence between layers.',
    'updates.coming_soon': 'Coming Soon',
    'updates.ai_voiceover': 'AI Voiceover (TTS)',
    'updates.voice_ai_pro': 'Voice AI Pro (Search + 22 Voices)',
    'updates.voice_ai_pro_desc':
      'Intelligent search by tags (cartoon, slow, etc.), extended catalog to 22 voices and bilingual pre-listening (EN/ES) before adding to timeline.',
    'updates.transitions': 'Transition Effects',
    'updates.audio_mixing': 'Advanced Audio Mixing',
    'updates.filters_effects': 'Visual Filters & Effects',
    'updates.export_formats': 'New Export Formats',
    'updates.stt': 'Speech to Text (STT)',
    'updates.captions': 'Auto Captions Generation',
    'updates.script_to_video': 'AI Script to Video',
    'updates.link_to_video': 'AI Link to Video',
    'updates.i18n': 'Bilingual Support (EN/ES)',
    'updates.i18n_desc':
      'The entire interface is now available in both Spanish and English with instant switching.',
    'updates.ai_stability': 'AI Assistant Stability',
    'updates.ai_stability_desc':
      'Strategic migration to us-east4 and improved quota management. Better stability for heavy video processing and Gemini streaming.',
    'updates.corner_radius_fix': 'Border Radius Fix',
    'updates.corner_radius_fix_desc':
      'Fixed an issue that prevented correctly applying border radius with value 0, enforcing a minimum of 1.',
    'updates.ai_assets_desc':
      'LangChain-powered generation with Vision Proxy. Deep style transformation and improved visual consistency.',
    'updates.ai_highlights': 'AI Highlights v3.5 (LangChain)',
    'updates.ai_highlights_desc':
      'Advanced analysis with LangChain Vertex AI. Improved moment detection accuracy and clip duration control.',
    'updates.ai_captions': 'AI Auto-Captions v1',
    'updates.ai_captions_desc':
      'Generate and burn-in captions automatically using Deepgram and Gemini. Includes a new editor view and clip previews.',
    'updates.transitions_stability': 'Transitions Stability',
    'updates.transitions_stability_desc':
      'Resolved inconsistent behavior when adding transitions in complex track unions.',
    'updates.export_performance': 'Export Performance',
    'updates.export_performance_desc':
      'Some users may experience slow export times with long videos. We are optimizing our cloud renderers.',
    'updates.auth': 'Authentication Support',
    'updates.auth_desc':
      'Secure login via Google, GitHub, or Email powered by Supabase.',
    'updates.access_control': 'AI Access Control',
    'updates.access_control_desc':
      'AI features (Highlights, Assets, and Captions) are now protected by user permissions and real-time API Key validation with demo mode for visitors.',
    'updates.highlights_tuning': 'Precision Highlights Tuning',
    'updates.highlights_tuning_desc':
      'Local pre-processing with FFmpeg (frames + audio), drastically reducing token usage and speeding up Gemini analysis.',
    'updates.langchain_orchestration': 'LangChain Orchestration',
    'updates.langchain_orchestration_desc':
      'New LangChain-based infrastructure for more robust, modular, and scalable AI logic across the Studio.',
    'updates.vision_proxy': 'Vision Proxy (Antigravity)',
    'updates.vision_proxy_desc':
      'Intelligent reference image analysis that breaks pixel bias for true artistic style transformations.',
    'captions.back': 'Back',
    'captions.setup_title': 'Auto Subtitles',
    'captions.list_title': 'Edit Subtitles',
    'captions.regenerate': 'Regenerate Subtitles',
    'captions.no_audio_error':
      'The selected video does not have an audio track. Captions cannot be generated.',
    'captions.processing': 'Processing Audio...',
    'captions.system_message': 'System Message',
    'captions.transcription_error': 'Transcription Error',
    'captions.alignment_complete': 'Automatic alignment complete',
    'highlights.title': 'AI Highlights',
    'highlights.description':
      'Automatically extract the best moments from your long videos.',
    'highlights.upload_title': 'Upload a long video',
    'highlights.upload_subtitle':
      'AI will identify the best moments for shorts',
    'highlights.extract_btn': 'Identify Moments with AI',
    'highlights.processing': 'Analyzing video with Gemini...',
    'highlights.suggested': 'Suggested Moments',
    'highlights.add_all': 'Add all to timeline',
    'highlights.target_duration': 'Target duration per clip',
    'highlights.range_short': 'Short (5 to 30 seconds)',
    'highlights.range_medium': 'Medium (30 to 60 seconds)',
    'highlights.range_long': 'Long (1 to 3 minutes)',
    'highlights.url_label': 'Paste YouTube link',
    'highlights.url_placeholder': 'https://www.youtube.com/watch?v=...',
    'highlights.url_btn': 'Download & Analyze',
    'highlights.add_to_timeline': 'Add to Timeline',
    'highlights.status_init': 'Starting process...',
    'highlights.status_uploading': 'Uploading part to Gemini...',
    'highlights.status_analyzing': 'Gemini analyzing part...',
    'highlights.status_optimizing': 'Optimizing part...',
    'highlights.status_split': 'Video split into parts.',
    'highlights.status_complete': 'Analysis completed!',
    'highlights.change_video': 'Change',
    'highlights.minutes': 'minutes',
    'highlights.seconds': 'seconds',
    'highlights.status_finalizing': 'Finishing touches...',
    'highlights.status_wait': 'This may take a few moments...',
    'highlights.clips_count': 'clips',
    'highlights.status_verifying': 'Verifying permissions...',
    'highlights.status_loading_tools': 'Loading tools...',
    'highlights.status_trimming': 'Trimming minutes',
    'highlights.status_analyzing_range': 'Analyzing minutes',
    'highlights.status_quota_wait': 'Regional saturation. Retrying in',
    'highlights.status_breathing': 'Breathing to release quota...',
    'highlights.status_error': 'Error in segment',
    'highlights.demo_mode': 'Demo Mode',
    'highlights.demo_title': 'Protected Content',
    'highlights.demo_description':
      'This feature requires an active API key. Contact the developer for a personalized demonstration.',
    'highlights.current_features': 'Current Features',
    'highlights.overview': 'Overview',
    'updates.issues': 'Known Issues',
    'updates.transitions_fix': 'Transitions Issue',
    'updates.transitions_fix_desc':
      'Inconsistent behavior has been detected when adding transitions in complex unions. Our team is working on a high-priority solution.',
    'header.export': 'Export',
    'header.logout': 'Sign Out',
    'header.import_json': 'Import from JSON',
    'header.export_json': 'Export to JSON',
    'header.json_menu': 'JSON',
    'header.undo': 'Undo',
    'header.redo': 'Redo',
    'transitions.title': 'Transitions',
    'transitions.editing_union': 'Editing Union',
    'effects.title': 'Effects',
    'text.basic': 'Basic',
    'text.add_text': 'Add text',
    'text.add_captions': 'Add captions',
    'toolbox.project_name': 'Pablito Project',
    'toolbox.coming_soon': 'Coming soon...',
    'modal.ai_assets': 'AI Assets',
    'modal.ai_images': 'AI Image Generation',
    'menu.media': 'Media',
    'menu.audio': 'Audio',
    'menu.text': 'Text',
    'menu.image': 'Image',
    'menu.effects': 'Effects',
    'menu.transitions': 'Transitions',
    'menu.highlights': 'AI Highlights',
    'menu.captions': 'Captions',
    'menu.ai_assets': 'AI Assets',
    'menu.voice_ai': 'Voice AI',
    'voice_ai.description': 'Generate professional voiceovers with ElevenLabs.',
    'voice_ai.input_method': 'Input Method',
    'voice_ai.tts': 'Text to Speech',
    'voice_ai.sts': 'Speech to Speech',
    'voice_ai.content_script': 'Script Content',
    'voice_ai.content_audio': 'Reference Audio',
    'voice_ai.placeholder_text': 'Write the text for the AI to speak here...',
    'voice_ai.characters': 'characters',
    'voice_ai.volume': 'Master Volume',
    'voice_ai.upload_audio': 'Upload audio file',
    'voice_ai.select_voice': 'Select Voice',
    'voice_ai.settings': 'Settings',
    'voice_ai.generate': 'Generate Voiceover',
    'voice_ai.search_placeholder':
      'Search by name or tags (cartoon, animate...)',
    'voice_ai.generated_results': 'Generated Results',
    'voice_ai.add_to_timeline': 'Add to Timeline',
    'voice_ai.play': 'Play',
    'voice_ai.stop': 'Stop',
    'voice_ai.voice_desc_soft': 'Soft narration',
    'voice_ai.voice_desc_sarah': 'Mature and confident',
    'voice_ai.voice_desc_charlie': 'Energetic and cheerful',
    'voice_ai.voice_desc_george': 'Captivating storyteller',
    'voice_ai.voice_desc_alice': 'Professional educator',
    'voice_ai.voice_desc_matilda': 'Professional and serious',
    'voice_ai.voice_desc_will': 'Relaxed optimist',
    'voice_ai.voice_desc_eric': 'Smooth and trustworthy',
    'voice_ai.voice_desc_brian': 'Deep and resonant',
    'voice_ai.voice_desc_bill': 'Wise and mature voice',
    'voice_ai.voice_desc_roger': 'Classy and casual',
    'voice_ai.voice_desc_laura': 'Quirky and enthusiast',
    'voice_ai.voice_desc_callum': 'Husky trickster',
    'voice_ai.voice_desc_river': 'Relaxed and neutral',
    'voice_ai.voice_desc_harry': 'Fierce warrior',
    'voice_ai.voice_desc_liam': 'Energetic creator',
    'voice_ai.voice_desc_jessica': 'Playful and bright',
    'voice_ai.voice_desc_bella': 'Professional and bright',
    'voice_ai.voice_desc_chris': 'Charming and casual',
    'voice_ai.voice_desc_daniel': 'Steady broadcaster',
    'voice_ai.voice_desc_lily': 'Velvety actress',
    'voice_ai.voice_desc_adam': 'Dominant and firm',
    'ai_assets.title': 'AI Generator',
    'ai_assets.description': 'Generate unique images and videos using AI.',
    'ai_assets.upload_image': 'Upload Image',
    'ai_assets.upload_video': 'Upload Video',
    'ai_assets.generate_img': 'Generate Image',
    'ai_assets.generate_vid': 'Generate Video',
    'ai_assets.generate_btn': 'Generate',
    'ai_assets.type': 'Content Type',
    'ai_assets.type_image': 'Image',
    'ai_assets.type_video': 'Video',
    'ai_assets.select_source': 'Select a file to start',
    'ai_assets.generating': 'Generating content...',
    'ai_assets.add_to_project': 'Add to Project',
    'ai_assets.video_duration': 'Video Duration',
    'ai_assets.duration_1m': '1 minute',
    'ai_assets.duration_3m': '3 minutes',
    'ai_assets.duration_5m': '5 minutes',
    'ai_assets.style': 'Visual Style',
    'ai_assets.style_realistic': 'Realistic',
    'ai_assets.style_scientific_glow': 'Scientific Glow',
    'ai_assets.style_manga': 'Manga',
    'ai_assets.style_cinematic': 'Cinematic',
    'ai_assets.style_3d': '3D Render',
    'ai_assets.style_cyberpunk': 'Cyberpunk',
    'ai_assets.style_cartoon': 'Cartoon',
    'ai_assets.style_pixel': 'Pixel Art',
    'ai_assets.style_watercolor': 'Watercolor',
    'ai_assets.style_oil': 'Oil Painting',
    'ai_assets.style_sketch': 'Sketch',
    'ai_assets.style_vintage': 'Vintage',
    'ai_assets.aspect_ratio': 'Aspect Ratio',
    'ai_assets.quality': 'Quality',
    'ai_assets.quality_standard': 'Standard',
    'ai_assets.quality_high': 'High',
    'ai_assets.quality_hd': 'HD',
    'timeline.split': 'Split (S)',
    'timeline.duplicate': 'Duplicate (D)',
    'timeline.delete': 'Delete (Del)',
    'timeline.snapping': 'Snapping (N)',
    'chat.assistant': 'Assistant AI',
    'chat.mode_demo': 'Demo Mode',
    'chat.gemini': 'Powered by Gemini',
    'chat.how_assist': 'How can I assist you?',
    'chat.placeholder':
      'I can help you with timeline edits, effects, or explaining editor features.',
    'chat.thinking': 'Thinking...',
    'chat.type_message': 'Type a message...',
    'chat.inaccurate': 'AI assistant may provide inaccurate info',
    'chat.adding_text': 'Adding text',
    'chat.updating_style': 'Updating text style...',
    'chat.updating_media_style': 'Updating media style...',
    'text.apply_hardcoded': 'Apply Hardcoded Style',
    'image.apply_hardcoded': 'Apply Hardcoded Style (Image)',
    'login.subtitle': 'All the power of AI for your videos.',
    'login.title': 'AI Video Editor',
    'login.description': 'Sign in with your Google account to get started',
    'login.google': 'Continue with Google',
    'login.github': 'GitHub',
    'login.or_continue': 'OR CONTINUE WITH EMAIL',
    'login.email': 'Email Address',
    'login.password': 'Password',
    'login.signin': 'Sign In',
    'login.signup': 'Sign Up',
    'login.privacy': 'By signing in, you agree to our privacy policy.',
    'login.ai_notice':
      'Sign-in is required to maintain control over AI services, as they are not enabled for the general public.',
    'login.rights': 'All rights reserved.',
    'login.try_without_login': 'Try editor without signing in',
    'login.try_without_login_note': 'AI features will not be available',
    'header.guest_mode': 'Guest Mode',
    'header.sign_in': 'Sign In',
    'header.guest_banner': 'Sign in to unlock AI features',
    'header.shortcuts': 'Keyboard shortcuts',
    'shortcuts.title': 'Keyboard Shortcuts',
    'shortcuts.global': 'Global',
    'shortcuts.timeline': 'Timeline',
    'shortcuts.canvas': 'Canvas',
    'shortcuts.undo': 'Undo',
    'shortcuts.redo': 'Redo',
    'shortcuts.play_pause': 'Play / Pause',
    'shortcuts.delete': 'Delete',
    'shortcuts.duplicate': 'Duplicate',
    'shortcuts.copy': 'Copy',
    'shortcuts.paste': 'Paste',
    'shortcuts.select_all': 'Select all',
    'shortcuts.save_json': 'Save project (JSON)',
    'shortcuts.export_video': 'Export video',
    'shortcuts.deselect': 'Deselect',
    'shortcuts.split': 'Split at playhead',
    'shortcuts.zoom_in': 'Zoom in timeline',
    'shortcuts.zoom_out': 'Zoom out timeline',
    'shortcuts.go_to_start': 'Go to start',
    'shortcuts.go_to_end': 'Go to end',
    'shortcuts.prev_frame': 'Previous frame',
    'shortcuts.next_frame': 'Next frame',
    'shortcuts.move_up': 'Move up 1px',
    'shortcuts.move_down': 'Move down 1px',
    'shortcuts.move_left': 'Move left 1px',
    'shortcuts.move_right': 'Move right 1px',
    'shortcuts.move_5px': 'Move 5px',
  },
} as const;

export type TranslationKey = keyof typeof translations.es;
